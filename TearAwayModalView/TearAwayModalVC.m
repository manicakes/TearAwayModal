...
@property (nonatomic, strong) CancelButton* cancelButton;
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  self.cancelButton = [[CancelButton alloc] init];
  self.cancelButton.frame = CGRectMake(0, 0, 80, 80);
  self.cancelButton.center = CGPointMake(self.view.center.x, self.view.frame.size.height - 100);
...
- (void)setContentView:(UIView *)contentView {
  ...
  self.contentView.frame = CGRectInset(self.view.bounds, 5, 5);
  ...
  self.snap = [[UISnapBehavior alloc] initWithItem:self.contentView snapToPoint:self.view.center];
...
- (void)didPan:(UIPanGestureRecognizer*)pan {
  if (pan.state == UIGestureRecognizerStateBegan) {
    ...
    if (!self.wantsDismissal) {
      ...
      CGFloat translationDist = sqrtf(translation.x * translation.x + translation.y * translation.y);
      self.dismissProgress = MIN(1.0, translationDist / self.dismissDistance);
      self.contentView.center = CGPointMake((self.dismissProgress) * (locationInView.x) + (1 - self.dismissProgress)*(self.view.center.x + translation.x),(self.dismissProgress) * (locationInView.y) + (1-self.dismissProgress)*(self.view.center.y + translation.y));
      CGFloat scaleFactor = MAX(0.3, 1 - self.dismissProgress * 0.7);
      self.contentView.transform = CGAffineTransformScale(self.contentTransform, scaleFactor, scaleFactor);
      //self.contentView.layer.cornerRadius = 15;
    } else {
      ...
      if (!self.overCancelButton && isCloseToCancelButton) {
        ...
        [UIView animateWithDuration:0.5 delay:0 usingSpringWithDamping:0.3 initialSpringVelocity:0 options:0 animations:^{
          self.cancelButton.transform = CGAffineTransformScale(self.cancelTransform, 1.3, 1.3);
        } completion:nil];
